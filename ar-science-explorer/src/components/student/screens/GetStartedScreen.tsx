  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate"
      ...
      <div className="relative w-32 h-32 mb-8">
        ...
        {[0, 1].map((i) => (
          <div key={i} className="absolute inset-0 rounded-full border opacity-60"
            style={{
              borderColor: ['hsl(var(--subject-chemistry))', 'hsl(var(--subject-biology))'][i],
            ...
            <div className="absolute w-2.5 h-2.5 rounded-full -top-1.5 left-1/2 -translate-x-1/2"
              style={{ background: ['hsl(var(--subject-chemistry))', 'hsl(var(--subject-biology))'][i] }} />
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-10">
        {(['biology', 'chemistry'] as const).map((s) => (
